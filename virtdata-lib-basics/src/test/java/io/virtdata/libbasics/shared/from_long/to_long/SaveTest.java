...
import io.virtdata.libbasics.shared.stateful.Clear;
...
public class SaveTest {
    ...
    public void testSaveUnaryString() {
        new Clear().apply(0L);
        io.virtdata.libbasics.shared.unary_string.Save saver = new io.virtdata.libbasics.shared.unary_string.Save("stringname");
        String passiveOutput = saver.apply("string1");
        assertThat(passiveOutput).isEqualTo("string1");
        io.virtdata.libbasics.shared.unary_string.Load loader = new io.virtdata.libbasics.shared.unary_string.Load("stringname");
        String loaded = loader.apply("3434434");
        assertThat(loaded).isEqualTo("string1");

        io.virtdata.libbasics.shared.unary_string.Load dloader = new io.virtdata.libbasics.shared.unary_string.Load("stringnam","e");
    ...
    public void testSaveUnaryDouble() {
        new Clear().apply(0L);
        io.virtdata.libbasics.shared.from_double.to_double.Save saver = new io.virtdata.libbasics.shared.from_double.to_double.Save("doublename");
        double v = saver.applyAsDouble(2345.678D);
        assertThat(v).isCloseTo(2345.678D, Offset.offset(0.0003D));
        io.virtdata.libbasics.shared.from_double.to_double.Load loader = new io.virtdata.libbasics.shared.from_double.to_double.Load("doublename");
