...
public class TrueShuffle {

    private final long max;
    private final Shuffle shuffle;

    @Example({"TrueShuffle(11,99)","Provide all values between 11 and 98 inclusive, in some order, then repeat"})
    public TrueShuffle(long min, long maxPlusOne) {
        this(min, maxPlusOne, Integer.MAX_VALUE);
    }

    @Example({"TrueShuffle(11,99,3)","Provide all values between 11 and 98 inclusive, in some different (and repeatable) order, then repeat"})
    public TrueShuffle(long min, long maxPlusOne, int bankSelector) {
        this.shuffle = new Shuffle(min, maxPlusOne*2, bankSelector);
        this.max = maxPlusOne;
    }

    public long applyAsLong(long register) {
        long result = shuffle.applyAsLong(register * 2);
        return result-max;
    }
